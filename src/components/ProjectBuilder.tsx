  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-5 gap-4 md:gap-5">
      ...
      <div className="md:col-span-3">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-medium text-gray-800 text-sm">費用グループを選ぶ</h3>
            <button
          ...
          {groups.length === 0 ? (
            <div className="text-center py-6 text-gray-400 text-sm">
              <p>費用グループがありません</p>
              <button onClick={() => setCreatingGroup(true)} className="mt-2 text-blue-500 hover:underline text-sm">
                今すぐ作る
              </button>
            </div>
          ) : (
              ...
              {groups.map(group => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex-1 text-sm font-medium text-gray-800 leading-tight">{group.name}</span>
      ...
      <div className="md:col-span-2 flex flex-col gap-3">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
          <h3 className="font-medium text-gray-800 text-sm mb-3">プロジェクト情報</h3>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
          <h3 className="font-medium text-gray-800 text-sm mb-2">
            選択中
            <span className="ml-1.5 text-xs font-normal text-gray-400">{selectedIds.length} グループ</span>
          </h3>

          {selectedGroups.length === 0 ? (
            <p className="text-xs text-gray-400 text-center py-3">グループを選んでください</p>
          ) : (
            <div className="space-y-1.5 mb-3">
              {selectedGroups.map((group, idx) => (
                <div key={group.id} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                  ...
                  <span className="flex-1 text-xs text-gray-800 font-medium truncate">{group.name}</span>
                  <span className="text-xs text-gray-500 shrink-0">{formatCurrency(calcGroupTotal(group))}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={handleSave}
            className="flex-1 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 active:bg-blue-800 transition-colors font-semibold text-sm shadow-sm"
          >
            予算を確定して保存
          </button>
        </div>
